...
const ENGINEER_MOBILE_START_TRAVEL_ACTION = 'engineer_mobile.start_travel';
const ENGINEER_MOBILE_START_TRAVEL_PERMISSION = 'engineer_mobile.visit.start_travel';

const OPEN_START_TRAVEL_STATUSES = new Set([
  'scheduled',
  'rescheduled',
]);
...
function includesPermission(actor) {
  return Array.isArray(actor.permissions)
    && actor.permissions.includes(ENGINEER_MOBILE_START_TRAVEL_PERMISSION);
}
...
function evaluateEngineerMobileStartTravelAction(options = {}) {
  ...
  if (!OPEN_START_TRAVEL_STATUSES.has(appointmentStatus(appointment))) {
