...
const ENGINEER_MOBILE_REQUIRED_PERMISSIONS = Object.freeze([
  'engineer_mobile.tasks.read',
  'engineer_mobile.tasks.read.assigned',
  'engineer_mobile.workbench.access',
]);

const ENGINEER_MOBILE_ALLOWED_ROLES = Object.freeze([
  'admin',
  'dispatch_assistant',
  'engineer',
  'supervisor',
...
const ENGINEER_MOBILE_PERMISSION_DENIED_RESPONSE = Object.freeze({
  status: 'deny',
...
function hasEngineerMobileTaskReadPermission(permissions) {
  const permissionSet = new Set(safePermissions(permissions));

  return ENGINEER_MOBILE_REQUIRED_PERMISSIONS.some((permission) => permissionSet.has(permission));
}
...
function safeDeny() {
  return {
    allowed: false,
    statusCode: 403,
    responseBody: { ...ENGINEER_MOBILE_PERMISSION_DENIED_RESPONSE },
  };
}
...
function buildEngineerMobilePermissionContext(req, options = {}) {
  ...
  if (!hasEngineerMobileTaskReadPermission(permissions)) {
    return safeDeny();
  }
...
function createEngineerMobilePermissionMiddleware(options = {}) {
  return function engineerMobilePermissionMiddleware(req, res, next) {
    const decision = buildEngineerMobilePermissionContext(req, options);
