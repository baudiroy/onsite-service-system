# Task1796 Engineer Mobile Arrive Action Policy / No DB No Global Mount

## Purpose

Task1796 adds one bounded pure runtime policy for `engineer_mobile.arrive`.

The policy only decides whether an assigned engineer may mark arrival for an appointment snapshot. It does not persist state, expose an API route, mount a route globally, send provider messages, create completion reports, or publish any customer-visible effect.

## Modified Files

- `src/engineerMobile/engineerMobileArriveActionPolicy.js`
- `tests/engineerMobile/engineerMobileArriveActionPolicy.unit.test.js`
- `tests/engineerMobile/engineerMobileArriveActionPolicyBoundary.static.test.js`
- `docs/task-1796-engineer-mobile-arrive-action-policy-no-db-no-global-mount.md`
...
- actor has `engineer_mobile.visit.arrive`.
- appointment status is still open for arrival: `scheduled` or `rescheduled`.
- travel has started by mobile visit status, visit status, or travel-start timestamp.
...
- allowed arrival when travel started by `mobileVisitStatus: "traveling"`.
- allowed arrival when travel started by `visitStatus: "traveling"`.
- allowed arrival when travel started by `travelStartedAt`.
- missing actor.
- missing appointment.
...
```bash
node --test tests/engineerMobile/engineerMobileArriveActionPolicy.unit.test.js
node --test tests/engineerMobile/engineerMobileArriveActionPolicyBoundary.static.test.js
node --test tests/engineerMobile/engineerMobileArriveActionPolicy.unit.test.js tests/engineerMobile/engineerMobileArriveActionPolicyBoundary.static.test.js
npm run check
git diff --check -- src/engineerMobile/engineerMobileArriveActionPolicy.js tests/engineerMobile/engineerMobileArriveActionPolicy.unit.test.js tests/engineerMobile/engineerMobileArriveActionPolicyBoundary.static.test.js docs/task-1796-engineer-mobile-arrive-action-policy-no-db-no-global-mount.md
```
