# Task2054 Customer-facing Service Report Case Reference Value Contract / No DB No Smoke

## Current baseline

- Baseline commit before this task: `5276e961f82a3579ab1970ca6c63638df6923446`
- Branch: `main`
- Upstream: `origin/main`
- Scope: Customer-facing service report `caseReference` display value filtering only.

## Findings

Task2051 through Task2053 filtered malformed `serviceStatus`, `engineerDisplayName`, and `appointmentWindow` display values before they can be emitted in the customer-facing service report DTO.

Task2054 applies the same bounded display-value contract to the optional `caseReference` field. Before this task, any non-empty string from `caseReference`, `case_display_id`, `caseDisplayId`, or `customer_case_reference` could be emitted. That made malformed rows harder to catch when URL-like text, token markers, SQL text, DB URLs, paths, or other internal sentinels were accidentally placed in the customer-facing case reference column.

## Runtime change

`src/customerAccess/customerServiceReportProjectionService.js` now emits `caseReference` only when the value is a safe customer display string.
...
Safe ordinary case references, such as `CASE-001`, remain unchanged.

## Coverage added

The no-DB tests now explicitly cover:

- Direct projection service rows with malformed case references.
- Full mounted customer-facing service report route rows with malformed case references.
- Existing full-route and direct projection handler DTO equivalence after case reference filtering.
- Safe omission without leaking the malformed case reference sentinel.

## Behavior preserved

- Existing row-level publication and customer-visible policy guard remains unchanged.
- Existing service status, engineer display-name, appointment window, and completion time filtering remains unchanged.
...
- `find tests/customerAccess -name '*.js' -print0 | xargs -0 node --test`
  - Result: PASS, 792/792 tests passing.
- `npm run check`
  - Result: PASS.
- `git diff --check`
  - Result: PASS.
