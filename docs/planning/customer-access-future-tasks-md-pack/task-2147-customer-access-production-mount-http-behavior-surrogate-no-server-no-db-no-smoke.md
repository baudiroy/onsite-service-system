...
Phase A — Production mount behavior
...
Add synthetic HTTP behavior coverage for mounted production composition without starting server and without DB.
...
- `node --test tests/customerAccess/customerAccessProductionMount.http-behavior.unit.test.js tests/customerAccess/customerAccessMountedRouteAllow.unit.test.js tests/customerAccess/customerAccessMountedRouteSafeDeny.unit.test.js`
- `git diff --check`
- `git status --short --branch`

## Required tests / acceptance behavior

Validate accepted routes, unsupported method/path no dispatch, safe-deny envelope unchanged, optional audit side-channel remains optional, no raw leakage.
