...
## Task1947 — Open Intake Reporter Customer Billing DTO Guard

Phase: Phase 14 — Open Repair Intake, Brand API, and Controlled Import

Purpose:
- Harden external intake DTO separation for reporter/customer/billing contacts.

Allowed work:
- Implement the smallest bounded runtime/source/test changes required by the task title and description.
- Use injected dependencies and synthetic tests where possible before any real DB/provider target.
...
- Do not use real provider keys, real provider calls, or live charges unless this task is separately approved for a sandbox/live target.
