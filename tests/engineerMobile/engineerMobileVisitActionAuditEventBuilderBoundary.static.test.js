...
const BUILDER_FILE = 'src/engineerMobile/engineerMobileVisitActionAuditEventBuilder.js';
const UNIT_TEST_FILE = 'tests/engineerMobile/engineerMobileVisitActionAuditEventBuilder.unit.test.js';
const BOUNDARY_TEST_FILE = 'tests/engineerMobile/engineerMobileVisitActionAuditEventBuilderBoundary.static.test.js';
const TASK_DOC = 'docs/task-1822-engineer-mobile-visit-action-audit-event-builder-no-db.md';
...
test('Task1822 allowed files exist', () => {
...
test('audit event builder has no runtime imports', () => {
  const source = read(BUILDER_FILE);

  assert.deepEqual(requireSpecifiers(source), []);
});

test('audit event builder stays isolated from forbidden runtimes', () => {
  const source = read(BUILDER_FILE);
...
test('audit event builder does not execute persistence provider route or completion calls', () => {
  const source = read(BUILDER_FILE);
...
test('audit event builder remains pure and synchronous', () => {
  const source = read(BUILDER_FILE);
...
test('Task1822 doc records required pure audit event builder boundaries', () => {
