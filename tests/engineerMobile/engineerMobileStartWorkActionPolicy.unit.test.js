...
function actor(overrides = {}) {
  return {
    id: 'eng_task_1798',
    organizationId: 'org_task_1798',
    permissions: [ENGINEER_MOBILE_START_WORK_PERMISSION],
    ...overrides,
  };
}
...
function appointment(overrides = {}) {
  return {
    appointmentId: 'apt_task_1798',
    organizationId: 'org_task_1798',
    assignedEngineerId: 'eng_task_1798',
    status: 'scheduled',
...
function evaluate(overrides = {}) {
  return evaluateEngineerMobileStartWorkAction({
    actor: actor(overrides.actor),
    appointment: appointment(overrides.appointment),
    now: '2026-05-28T09:30:00.000Z',
  });
}
...
function assertDenied(result, reasonCode) {
  ...
  assert.equal(result.action, ENGINEER_MOBILE_START_WORK_ACTION);
  assert.equal(result.permission, ENGINEER_MOBILE_START_WORK_PERMISSION);
...
function assertAllowed(result) {
  ...
  assert.equal(result.subject.actorId, 'eng_task_1798');
  assert.equal(result.subject.appointmentId, 'apt_task_1798');
  assert.equal(result.subject.organizationId, 'org_task_1798');
  assert.equal(result.auditIntent.allowed, true);
  assert.equal(result.auditIntent.occurredAt, '2026-05-28T09:30:00.000Z');
}
...
test('allows same-org assigned engineer with start-work permission when mobile visit status is arrived', () => {
...
test('allows when arrived by visit status arrived', () => {
  const result = evaluate({
    appointment: {
...
test('allows when arrived by arrivedAt', () => {
  const result = evaluate({
    appointment: {
      mobileVisitStatus: undefined,
      arrivedAt: '2026-05-28T09:20:00.000Z',
    },
  });
...
test('denies missing actor', () => {
  const result = evaluateEngineerMobileStartWorkAction({
...
test('denies when arrival evidence is missing', () => {
  assertDenied(evaluate({
    appointment: {
      mobileVisitStatus: undefined,
...
test('denies already working appointment', () => {
  assertDenied(evaluate({
    appointment: {
      arrivedAt: '2026-05-28T09:20:00.000Z',
      workStartedAt: '2026-05-28T09:25:00.000Z',
    },
  }), 'already_working');
});
...
test('denies completion-report boundary indicators', () => {
  for (const indicator of [
    ...
    { createReport: true },
    { publishReport: true },
    { fieldServiceReportId: 'fsr_task_1798' },
