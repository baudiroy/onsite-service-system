...
const ADAPTER_FILE = 'src/engineerMobile/engineerMobileVisitActionAuditWriterAdapter.js';
const UNIT_TEST_FILE = 'tests/engineerMobile/engineerMobileVisitActionAuditWriterAdapter.unit.test.js';
const BOUNDARY_TEST_FILE = 'tests/engineerMobile/engineerMobileVisitActionAuditWriterAdapterBoundary.static.test.js';
const TASK_DOC = 'docs/task-1824-engineer-mobile-visit-action-audit-writer-adapter-injected-audit-event-writer-no-db.md';
...
test('Task1824 allowed files exist', () => {
...
test('audit writer adapter imports only the accepted audit event builder', () => {
  const source = read(ADAPTER_FILE);

  assert.deepEqual(requireSpecifiers(source), ['./engineerMobileVisitActionAuditEventBuilder']);
});

test('audit writer adapter stays isolated from forbidden runtimes', () => {
  const source = read(ADAPTER_FILE);
...
test('audit writer adapter does not execute real audit persistence provider route or completion calls', () => {
  const source = read(ADAPTER_FILE);
...
test('audit writer adapter remains synchronous and injected-only', () => {
  const source = read(ADAPTER_FILE);

  assert.doesNotMatch(source, /async\s+function/);
  assert.doesNotMatch(source, /Promise\./);
  assert.doesNotMatch(source, /await\s+/);
  assert.match(source, /auditEventWriter\.record/);
  assert.match(source, /buildEngineerMobileVisitActionAuditEvent/);
});

test('Task1824 doc records required injected audit event writer boundaries', () => {
