...
test('server explicit async pool service report full route omits malformed engineer display name', async () => {
  const queryCalls = [];
  const rows = allAllowRows();
...
test('server explicit async pool service report full route omits malformed case reference', async () => {
  const queryCalls = [];
  const rows = allAllowRows();
