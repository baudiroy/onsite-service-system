...
test('production mount composition adapter has no global app server public route db env provider ai or billing imports', () => {
  ...
  const forbiddenSourcePattern = /src\/app|src\/server|public\.routes|routes\/index|customerAccessRouteRegistry|customerAccessDbClientFactory|customerAccessReadOnlyDbConnector|createCustomerAccessDbAdapter|process\.env|DATABASE_URL|Zeabur|provider|OpenAI|RAG|model|billing|payment|settlement|invoice|routeRegistry|globalRoute/i;
  const forbiddenSpecifierPattern = /app|server|public\.routes|routes\/index|RouteRegistry|DbClientFactory|ReadOnlyDbConnector|DbAdapter|env|zeabur|provider|openai|rag|model|billing|payment|settlement|invoice|registry/i;
...
test('production mount composition adapter has no import side effects or runtime startup/db execution calls', () => {
  ...
  assert.doesNotMatch(source, /fetch\s*\(|axios|http\.request|https\.request|child_process|exec\s*\(|spawn\s*\(|fs\.|readFile|writeFile|appendFile|console\.|XMLHttpRequest|WebSocket/i);
...
test('production mount composition adapter keeps registration summary delegated and does not synthesize alternate routes', () => {
  const source = read(adapterPath);

  assert.match(source, /registerCustomerAccessRoutes\(options\.router,\s*\{\s*dbClient:\s*options\.dbClient,\s*repository:\s*options\.repository,\s*auditWriter:\s*options\.auditWriter,\s*\}\)/);
  assert.doesNotMatch(source, /registered:\s*true|registered:\s*false|messageKey|customerVisible|reasonCode/);
  assert.doesNotMatch(source, /\/customer-access|service-report|method:\s*['"]GET['"]/);
  assert.doesNotMatch(source, /\.push\s*\(|routes\s*:\s*\[/);
});
