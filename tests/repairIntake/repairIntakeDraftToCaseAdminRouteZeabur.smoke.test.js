...
function requireDatabaseUrl() {
  ...
  if (!databaseUrl) {
    throw new Error('DATABASE_URL is required for the Zeabur admin route smoke test and was not printed.');
  }
...
test('Zeabur DB smoke posts protected admin Draft to Case route and cleans unique rows', async (t) => {
  const pool = new Pool({
  ...
  const caseNo = uniqueText('TASK1669_CASE');
  const idempotencyKey = uniqueText('task1669_idem');
  const requestId = uniqueText('task1669_req');
  const sourceRef = uniqueText('task1669_source');
  const safeRequestFingerprint = uniqueText('task1669_fp');
  ...
  try {
    ...
    await pool.query(
      ...
      [organizationId, uniqueText('task1669_org'), 'Task1669 Test Organization'],
    );
    ...
    await pool.query(
      ...
      [
        customerId,
        organizationId,
        'Task1669 Test Customer',
        uniqueText('task1669_mobile'),
        'Task1669 City',
        'Task1669 Test Address',
        'website',
        JSON.stringify({ testScope: 'task1669' }),
      ],
    );
    ...
    await pool.query(
      ...
      [
        ...
        tenantId,
        sourceRef,
        JSON.stringify({ title: 'Task1669 safe admin route intake summary' }),
        JSON.stringify({ testScope: 'task1669', caseNo }),
        JSON.stringify([]),
      ],
    );
    ...
    const runtimePorts = {
      ...
      planningPolicy: {
        async planCaseFromDraft(input) {
          return {
            status: 'planned',
            reasonCode: 'TASK1669_ADMIN_ROUTE_PLAN_READY',
            candidate: {
              ...
              customerId,
              source: 'web',
              brand: 'Task1669 Brand',
              productType: 'Task1669 Product',
              modelNo: 'Task1669 Model',
              problemDescription: 'Task1669 safe issue summary',
              serviceType: 'onsite',
              priority: 'normal',
              serviceRegion: 'Task1669 Region',
              metadata: {
                testScope: 'task1669',
                sourceRef,
              },
        ...
        async createCaseFromDraft(input) {
          ...
          await pool.query(
            ...
            [
              ...
              actorId,
              requestId,
              JSON.stringify({ testScope: 'task1669', routeLevel: 'admin' }),
            ],
          );
      ...
      auditPort: {
        async recordDraftToCaseDecision(input) {
          ...
          return {
            ...
            tenantId,
            caseId,
            reasonCode: 'TASK1669_ADMIN_ROUTE_AUDIT_RECORDED',
            metadata: {
              testScope: 'task1669',
            },
          };
    ...
    const appRouter = createAppRouter({
      ...
      repairIntakeDraftToCaseRuntimePorts: runtimePorts,
    });
    ...
    const response = await dispatchRoute(route, {
      ...
      body: {
        ...
        idempotencyKey,
        organizationId: uniqueText('task1669_body_org_should_not_win'),
        permissionContext: {
    ...
    assert.equal(insertedCase.rows[0].brand, 'Task1669 Brand');
    assert.equal(insertedCase.rows[0].case_type, 'repair');
    assert.equal(insertedCase.rows[0].metadata.testScope, 'task1669');
    ...
    assert.equal(conversion.rows[0].actor_id, actorId);
    ...
    assert.equal(audit.rows[0].actor_id, actorId);
